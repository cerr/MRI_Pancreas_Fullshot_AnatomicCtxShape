...
def main(args):
    ...
    # Identify inputs
    keyword = 'SCAN'
    src_img_path = find('*_MR SCAN_last_scan_3D.nii.gz', inputScanPath)
    src_mask_path = find('*_MR SCAN_first_4D.nii.gz', inputMaskPath)
    target_img_path = find('*_MR SCAN_first_scan_3D.nii.gz', inputScanPath)

    if not src_img_path:
        raise ValueError('Missing src img ''*_MR SCAN_last_scan_3D.h5''')
    else:
        print('INFO: src_img_path : ' + src_img_path[0])
    if not src_mask_path:
        raise ValueError('Missing mask ''*_MR SCAN_last_4D.h5''')
    else:
        print('INFO: src_mask_path : ' + src_mask_path[0])
    if not target_img_path:
        raise ValueError('Missing target img ''*_MR SCAN_first_scan_3D.h5''')
    else:
