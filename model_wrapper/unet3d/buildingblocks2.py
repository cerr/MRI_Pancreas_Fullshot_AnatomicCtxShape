...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3,
        ...
        if basic_module == DoubleConv:
            ...
            # (D_out?=?(D_in???1)?¡Á??stride[0]???2?¡Á??padding[0]?+??kernel_size[0]?+??output_padding[0])
...
class FinalConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, order='crg', num_groups=8):
        ...
        # in the last layer a 1¡Á1 convolution reduces the number of output channels to out_channels
