...
def save_checkpoint_epoch(state, epoch, checkpoint_dir, logger=None):
    """Saves model and training parameters at '{checkpoint_dir}/last_checkpoint.pytorch'.
    If is_best==True saves '{checkpoint_dir}/best_checkpoint.pytorch' as well.
    ...
    if not os.path.exists(checkpoint_dir):
        log_info(
            f"Checkpoint directory does not exists. Creating {checkpoint_dir}")
        os.mkdir(checkpoint_dir)
    sv_name_=str(epoch)+'_epoch_checkpoint.pytorch'
    last_file_path = os.path.join(checkpoint_dir, sv_name_)
    log_info(f"Saving Epoch checkpoint to '{last_file_path}'")
    torch.save(state, last_file_path)
